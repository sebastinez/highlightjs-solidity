...
let SOL_NUMBER_RE: string | RegExp =
...
function solAposStringMode(hljs: HLJSApi) {
  return hljs.inherit(
...
function solQuoteStringMode(hljs: HLJSApi) {
  return hljs.inherit(
...
function baseAssembly(hljs: HLJSApi): Mode {
  ...
  const SOL_ASSEMBLY_LEXEMES_RE = /[A-Za-z_$][A-Za-z_$0-9.]*/;

  const SOL_ASSEMBLY_TITLE_MODE = hljs.inherit(hljs.TITLE_MODE, {
